...
const Animaliak= ({id,name, kokapena, img, mota, animaliType, item})=>{
    ...
    if(mota==="adopzio"){
      ...
      return <Link to={`/Animalia_galduta/${id}`} className="w-1/4  max-w-80 mb-8 mx-5">
          <img src={link} alt={name} className="rounded-t-3xl h-[218.2px] w-[320px]"/>
      ...
      return <Link to={`/Edit_animalia/${id}`} state={{item: item}} className=" max-w-64  mb-8 mt-8 mx-5">
